function [ber,snr]=bersimBPSK(SNRdB,trials)
R=1/2;
snr=SNRdB;
...
for z=1:length(trials)%length(SNRdB)
    trial=trials(z);
    for run=1:trial
      msg = randi([0 1],1,blocklength); % Generate a random binary message
      codeword = msg;%encode(enc,msg); % Encode the message
      ...
      receivedsig = modulatedsig+sqrt(var(z)).*randn(1,blocklength);
      ...
      decodedmsg = llr;%decode(dec, llr);
      decodedmsg(find(llr<0))=1;
      decodedmsg(find(llr>0))=0;
...
%       parity_checks_violated = num2str(sum(dec.FinalParityChecks));
    ...
    ber(z)=av_er(z)/blocklength;%32400; %for R=1/3-->ber(z)=av_er(z)/21600;
end